...
import { SportType, CourtGroup, Court, PeakHourPricing } from '../interfaces/field';
import { NAMING_METHODS, DEFAULT_PEAK_HOURS } from '../constants/sportTypes';
...
export const CourtGroupForm: React.FC<CourtGroupFormProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    peakHourPricing: initialData?.courts[0]?.peakHourPricing || [DEFAULT_PEAK_HOURS]
  });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 bg-gray-50 p-6 rounded-lg">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">
          Số tiền tăng thêm khi giờ cao điểm
        </label>
        {formData.peakHourPricing.map((pricing, index) => (
          <div key={index} className="grid grid-cols-3 gap-2 mb-2">
            <TimePicker
              format="HH:mm"
              value={pricing.startTime ? dayjs(pricing.startTime, 'HH:mm') : null}
              onChange={(time) => handlePeakHourChange(index, 'startTime', time?.format('HH:mm') || '')}
              className="w-full"
            />
            <TimePicker
              format="HH:mm"
              value={pricing.endTime ? dayjs(pricing.endTime, 'HH:mm') : null}
              onChange={(time) => handlePeakHourChange(index, 'endTime', time?.format('HH:mm') || '')}
              className="w-full"
            />
            <input
              type="number"

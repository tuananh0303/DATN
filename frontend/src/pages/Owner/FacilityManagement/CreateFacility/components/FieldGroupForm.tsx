...
const FieldGroupForm: React.FC<FieldGroupFormProps> = ({
  ...
  useEffect(() => {
    if (open) {
      ...
      const initialSportIds = selectedSportIds.length > 0 
        ? selectedSportIds 
        : (sport.id === 999 
          ? [] // Start with empty selection for composite sport
          : [sport.id]);
      ...
      setTimeout(() => {
        ...
        const isEditing = editingFieldGroup && Object.keys(editingFieldGroup || {}).length > 0;
        let editingData: EditingData = {};
        let numberOfPeaks = 0; // Mặc định là 0 thay vì 1
        
        // Adjust fields based on existing data
        if (isEditing && editingFieldGroup) {
          // Use the editingFieldGroup prop directly
        ...
        // Set all form values
        form.setFieldsValue({
          name: isEditing && editingFieldGroup?.name ? editingFieldGroup.name : autoGeneratedName,
          dimension: isEditing && editingFieldGroup?.dimension ? editingFieldGroup.dimension : finalDimension,
          surface: isEditing && editingFieldGroup?.surface ? editingFieldGroup.surface : finalSurface,
          basePrice: isEditing && editingFieldGroup?.basePrice !== undefined ? editingFieldGroup.basePrice : defaultPrice,
          numberOfPeaks: numberOfPeaks,
          sportIds: initialSportIds,
          fieldCount: isEditing && editingFieldGroup?.fields ? editingFieldGroup.fields.length : 1,
          useCustomName: isEditing && editingFieldGroup?.name && !editingFieldGroup.name.includes(`Nhóm ${fieldCount}`) ? true : false,
          
          // Set peak times and price increases
          peakTime1: peakTime1,
          peakStartTime1: peakTime1 ? peakTime1[0] : null,
          peakEndTime1: peakTime1 ? peakTime1[1] : null,
          priceIncrease1: isEditing && editingFieldGroup?.priceIncrease1 !== undefined ? editingFieldGroup.priceIncrease1 : 0,
          
          peakTime2: peakTime2,
          peakStartTime2: peakTime2 ? peakTime2[0] : null,
          peakEndTime2: peakTime2 ? peakTime2[1] : null,
          priceIncrease2: isEditing && editingFieldGroup?.priceIncrease2 !== undefined ? editingFieldGroup.priceIncrease2 : 0,
          
          peakTime3: peakTime3,
          peakStartTime3: peakTime3 ? peakTime3[0] : null,
          peakEndTime3: peakTime3 ? peakTime3[1] : null,
          priceIncrease3: isEditing && editingFieldGroup?.priceIncrease3 !== undefined ? editingFieldGroup.priceIncrease3 : 0,
        });
  ...
  const removePeakTime = (peakNumber: number) => {
    if (peakNumber === 1) {
      // When removing peak 1, remove all peak times
      form.setFieldsValue({
        peakStartTime1: null,
        peakEndTime1: null,
        peakStartTime2: null,
              ...
              <Form.Item label="Loại hình thể thao">
                {isMultiSport ? (
                  <>
                    ...
                    {/* Only show selected sports if there are any */}
                    {sport.id === 999 && selectedSports.length > 0 && (
                      <div className="mt-2">
                        <Text type="secondary">
